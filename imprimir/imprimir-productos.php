...
function Footer()
{
	...
	$sql = $bd->query("SELECT a.Id_Producto, a.Precio, e.descripcion,a.Descripcion, r.Nombre FROM productos as a inner JOIN categorias as e inner JOIN proveedores as r ON e.Id_categoria=a.Id_categoria and r.Id_Proveedor=a.Id_Proveedor WHERE a.Id_Categoria and a.Id_Proveedor AND a.estado = 1");
	...
	{
		//ENCABEZADOS DE LA TABLA
		$pdf->Row(array('PRODUCTO', 'PRECIO', utf8_decode('CATEGORÍA'), 'PROVEEDOR'));
	}
			...
			if ($bd->rows($sql) > 0) {
				...
				while ($row = $bd->recorrer($sql)) {
					...
					$pdf->SetFont('helvetica','',9);
						...
						$pdf->SetFillColor(255);
		                $pdf->Row(array($row['Descripcion'], $row['Precio'], $row['descripcion'], $row['Nombre']));
				}
			} else {
