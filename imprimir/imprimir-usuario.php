...
function Footer()
{
	...
	$sql = $bd->query("SELECT nombre, apellido, email, usuario from usuario where estado = 1");
	...
	{
		//ENCABEZADOS DE LA TABLA
		$pdf->Row(array('NOMBRES', 'APELLIDOS', 'EMAIL', 'USUARIO'));
	}
			...
			if ($bd->rows($sql) > 0) {
				...
				while ($row = $bd->recorrer($sql)) {
					...
					$pdf->SetFont('helvetica','',9);
						...
						$pdf->SetFillColor(255);
		                $pdf->Row(array($row['nombre'], $row['apellido'], $row['email'], $row['usuario']));
				}
			} else {
