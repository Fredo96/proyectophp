...
function Footer()
{
	...
	$sql = $bd->query("SELECT Nombre, Direccion, Telefono from proveedores where estado = 1");
	...
	$pdf->Row(array('PROVEEDORES'));

    //Encabezados de la tabla
	for($i=0;$i<1;$i++)
	{
		//ENCABEZADOS DE LA TABLA
		$pdf->Row(array('NOMBRES', utf8_decode('DIRECCIÓN'), utf8_decode('TELÉFONO')));
	}
			...
			if ($bd->rows($sql) > 0) {
				...
				while ($row = $bd->recorrer($sql)) {
					...
					$pdf->SetFont('helvetica','',9);
						...
						$pdf->SetFillColor(255);
		                $pdf->Row(array($row['Nombre'], $row['Direccion'], $row['Telefono']));
				}
			} else {
