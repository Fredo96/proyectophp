...
function Footer()
{
	...
	$sql = $bd->query("SELECT a.Id_venta, a.Cantidad, e.Fecha, r.Descripcion FROM ventas as a inner JOIN factura as e inner JOIN productos as r ON e.Id_Factura=a.Id_Factura and r.Id_Producto=a.Id_Producto WHERE a.Id_Factura and a.Id_Producto AND a.estado = 1");
	...
	$pdf->Row(array('VENTAS'));

    //Encabezados de la tabla
	for($i=0;$i<1;$i++)
	{
		//ENCABEZADOS DE LA TABLA
		$pdf->Row(array('FECHA', utf8_decode('DESCRIPCIÓN'), 'CANTIDAD'));
	}
			...
			if ($bd->rows($sql) > 0) {
				...
				while ($row = $bd->recorrer($sql)) {
					...
					$pdf->SetFont('helvetica','',9);
						...
						$pdf->SetFillColor(255);
		                $pdf->Row(array($row['Fecha'], $row['Descripcion'], $row['Cantidad']));
				}
			} else {
