...
$pagination->rowCount("SELECT * FROM proveedores WHERE estado = 1 and Nombre LIKE '%$search%' ORDER BY  Nombre");
$pagination->config(3, 15);
$sql = "SELECT * FROM proveedores WHERE estado = 1 and Nombre LIKE '%$search%' ORDER BY Nombre LIMIT $pagination->start_row, $pagination->max_rows";
...
{
$pagination->rowCount("SELECT * FROM proveedores where  estado = 1 ORDER BY Nombre");
$pagination->config(3, 15);
$sql = "SELECT * FROM proveedores WHERE estado = 1 ORDER BY Nombre LIMIT $pagination->start_row, $pagination->max_rows";
$query = $connection->prepare($sql);
$query->execute();
$model = array();
while($rows = $query->fetch())
...
if ($pagination->rowCount( "SELECT * FROM proveedores WHERE estado = 1 and Nombre LIKE '%$search%'")>0 || $query->rowCount($sql)>0){
?>
                               ...
                               <td> &nbsp;<input type="submit"  value="BUSCAR" class="btn btn-primary"> &nbsp; <a href="../imprimir/imprimir-proveedor.php" target="_blank"  class="btn btn-info" title="Imprimir Reporte">IMPRIMIR</a></td>
                                </div></td>
                          </form></tr>
                          </table>
...
<table class='table table-striped table-hover  form-group responsive' >
    ...
    foreach($model as $row)
    {
    	//mensaje para verificar si realmente desea borrar o no
		                        ...
		                        $url = "../consultar/ver-proveedor.php";
		                        $variables = "campo=Id_Proveedor&valorCampo=".$row['Id_Proveedor']."&tabla=proveedores&url=".$url;
						        ...
						        echo '<td width=20% style="text-align:center;">
						        		<a class="btn btn-success" href="../modificar/modificar-proveedor.php?id='.$row['Id_Proveedor'].'" title="Editar Informaci&oacute;n"><span class="glyphicon glyphicon-pencil"></span> </a>
    ...
    }elseif(isset($_POST["search"]) && $pagination->rowCount( "SELECT * FROM proveedores WHERE estado = 1 and Nombre LIKE '%$search%'")==0) {
        # no Hay datos
