...
$pagination->rowCount("SELECT * FROM preguntasecreta WHERE estado = 1 and nombre LIKE '%$search%' ORDER BY  nombre");
$pagination->config(3, 15);
$sql = "SELECT * FROM preguntasecreta WHERE estado = 1 and nombre LIKE '%$search%' ORDER BY nombre LIMIT $pagination->start_row, $pagination->max_rows";
...
{
$pagination->rowCount("SELECT * FROM preguntasecreta where  estado = 1 ORDER BY nombre");
$pagination->config(3, 15);
$sql = "SELECT * FROM preguntasecreta WHERE estado = 1 ORDER BY nombre LIMIT $pagination->start_row, $pagination->max_rows";
$query = $connection->prepare($sql);
$query->execute();
$model = array();
while($rows = $query->fetch())
...
if ($pagination->rowCount( "SELECT * FROM preguntasecreta WHERE estado = 1 and nombre LIKE '%$search%'")>0 || $query->rowCount($sql)>0){
?>
...
<table class='table table-striped table-hover  form-group responsive' >
    ...
    foreach($model as $row)
    {
    	//mensaje para verificar si realmente desea borrar o no
		                        ...
		                        $url = "../consultar/ver-pregunta.php";
		                        $variables = "campo=idpreguntasecreta&valorCampo=".$row['idpreguntasecreta']."&tabla=preguntasecreta&url=".$url;

						        echo "<tr>";
						        echo "<td>".$row['idpreguntasecreta']."</td>";
						        echo "<td>".$row['nombre']."</td>";
						       
						        echo '<td width=20% style="text-align:center;">
						        		<a class="btn btn-success" href="../modificar/modificar-pregunta.php?id='.$row['idpreguntasecreta'].'" title="Editar Informaci&oacute;n"><span class="glyphicon glyphicon-pencil"></span> </a>
    ...
    }elseif(isset($_POST["search"]) && $pagination->rowCount( "SELECT * FROM preguntasecreta WHERE estado = 1 and nombre LIKE '%$search%'")==0) {
        # no Hay datos
