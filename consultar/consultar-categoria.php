...
$pagination->rowCount("SELECT * FROM categorias WHERE estado = 1 and descripcion LIKE '%$search%' ORDER BY  nombre");
$pagination->config(3, 15);
$sql = "SELECT * FROM categorias WHERE estado = 1 and descripcion LIKE '%$search%' ORDER BY descripcion LIMIT $pagination->start_row, $pagination->max_rows";
...
{
$pagination->rowCount("SELECT * FROM categorias where  estado = 1 ORDER BY descripcion");
$pagination->config(3, 15);
$sql = "SELECT * FROM categorias WHERE estado = 1 ORDER BY descripcion LIMIT $pagination->start_row, $pagination->max_rows";
$query = $connection->prepare($sql);
$query->execute();
$model = array();
while($rows = $query->fetch())
...
if ($pagination->rowCount( "SELECT * FROM categorias WHERE estado = 1 and descripcion LIKE '%$search%'")>0 || $query->rowCount($sql)>0){
?>
                               ...
                               <td> &nbsp;<input type="submit"  value="BUSCAR" class="btn btn-primary"> &nbsp; <a href="../imprimir/imprimir-categoria.php" target="_blank"  class="btn btn-info" title="Imprimir Reporte">IMPRIMIR</a></td>
                                </div></td>
                          </form></tr>
                          </table>
...
<table class='table table-striped table-hover  form-group responsive' >
    ...
    foreach($model as $row)
    {
    	//mensaje para verificar si realmente desea borrar o no
		                        ...
		                        $variables = "campo=Id_Categoria&valorCampo=".$row['Id_Categoria']."&tabla=clientes&url=".$url;
						        ...
						        echo '<td width=20% style="text-align:center;">
						        		<a class="btn btn-success" href="../modificar/modificar-categoria.php?id='.$row['Id_Categoria'].'" title="Editar Informaci&oacute;n"><span class="glyphicon glyphicon-pencil"></span> </a>
    ...
    }elseif(isset($_POST["search"]) && $pagination->rowCount( "SELECT * FROM categorias WHERE estado = 1 and descripcion LIKE '%$search%'")==0) {
        # no Hay datos
